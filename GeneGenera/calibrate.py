...
def calibrate_shape_effects(df: pd.DataFrame) -> list[dict]:
    ...
    for cluster in range(4):
        ...
        cluster_stats.append({
            "cluster": cluster,
            "ci": float(rel["ci_norm"]),
            "vci": float(rel["vci_norm"]),
            "lum_adj": float(rel["lum_adjacency_norm"]),
            "lum_cluster": float(rel["lum_intensity_norm"]),
            "mean_ci": float(mean_vals["ci_norm"]),
            "mean_lum_adj": float(mean_vals["lum_adjacency_norm"]),
        })
    ...
    for info, name in zip(cluster_stats, names):
        shape_effects.append({
            "name": name,
            "ci": round(info["ci"], 4),
            "vci": round(info["vci"], 4),
            "lum_adj": round(info["lum_adj"], 4),
            "lum_cluster": round(info["lum_cluster"], 4),
        })
...
def calibrate_service_weights(df: pd.DataFrame) -> dict:
    ...
    focus_mapping = {
        "facility_neighborhood_ha": "retail",
        "facility_sales_ha": "retail",
        "facility_office_ha": "office",
        "facility_education_ha": "education",
    }
    ...
    result = {
        "retail": {
            "neighborhood": round(fmt("retail", "facility_neighborhood_ha"), 4),
            "sales": round(fmt("retail", "facility_sales_ha"), 4),
            "office": round(fmt("retail", "facility_office_ha"), 4),
            "education": round(fmt("retail", "facility_education_ha"), 4),
        },
        "office": {
            "neighborhood": round(fmt("office", "facility_neighborhood_ha"), 4),
            "sales": round(fmt("office", "facility_sales_ha"), 4),
            "office": round(fmt("office", "facility_office_ha"), 4),
            "education": round(fmt("office", "facility_education_ha"), 4),
        },
        "education": {
            "neighborhood": round(fmt("education", "facility_neighborhood_ha"), 4),
            "sales": round(fmt("education", "facility_sales_ha"), 4),
            "office": round(fmt("education", "facility_office_ha"), 4),
            "education": round(fmt("education", "facility_education_ha"), 4),
        },
    }
    return result
...
def main() -> None:
    df = load_dataset()
    calibration = {
        "shape_effects": calibrate_shape_effects(df),
        "service_focus_weights": calibrate_service_weights(df),
        "green_pattern_weights": calibrate_green_weights(df),
    }
