...
DEFAULT_SERVICE_WEIGHTS = {
    "retail": {"neighborhood": 0.35, "sales": 0.35, "office": 0.20, "education": 0.10},
    "office": {"neighborhood": 0.25, "sales": 0.15, "office": 0.40, "education": 0.20},
    "education": {"neighborhood": 0.20, "sales": 0.10, "office": 0.20, "education": 0.50},
}
...
if BLOCK_STATS_PATH.exists():
    ...
    BUILT_CAPS = {
        "multi_family_ha": block_stats["built"].get("multi_family_ha", {}).get("max", 40.0) * 1.2,
        "facility_industrial_ha": block_stats["built"].get("facility_industrial_ha", {}).get("max", 40.0) * 1.2,
    }
...
class GeneDecoder:
    ...
    def decode(self, cell_genes: Iterable[CellGene], global_gene: GlobalGene) -> pd.DataFrame:
        ...
        for gene, far_raw in zip(genes, far_values):
            ...
            row = {
                "grid_id_main": gene.grid_id,
                "ci_norm": _clamp(ci_norm, 0.0, 1.0),
                "vci_norm": _clamp(vci_norm, 0.0, 1.5),
                "lum_norm": _clamp(lum_norm, 0.0, 1.0),
                "lum_adjacency_norm": _clamp(lum_adjacency, 0.0, 1.5),
                "lum_intensity_norm": _clamp(lum_intensity, 0.0, 1.0),
                "lum_proximity_norm": _clamp(lum_proximity, 0.0, 1.0),
                "gi_norm": _clamp(gi_norm, 0.0, 1.5),
                "li_norm": _clamp(li_norm, 0.0, 1.5),
                "single_family_ha": single_family_ha,
                "multi_family_ha": min(multi_family_ha, BUILT_CAPS["multi_family_ha"]),
                "facility_neighborhood_ha": fac_neigh,
                "facility_sales_ha": fac_sales,
                "facility_office_ha": fac_office,
                "facility_education_ha": fac_education,
                "facility_industrial_ha": min(
                    fac_industrial, BUILT_CAPS["facility_industrial_ha"]
                ),
                "parks_green_ha": parks_green_ha,
                "water_area_ha": water_area_ha,
                "road_area_ha": min(road_area, ROAD_CAP),
                "subway_influence_ha": _clamp(subway_influence, 0.0, CELL_AREA_HA),
                "bus_routes_cnt": bus_routes,
            }
            rows.append(row)
