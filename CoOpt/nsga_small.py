...
class SimpleNSGA:
    ...
    def _serialize_genes(self, ind: Individual) -> dict:
        ...
        return {
            "ind_id": ind.ind_id,
            "cells": [serialize_cell(cell) for cell in ind.cells],
            "global_gene": asdict(ind.global_gene),
        }
    ...
    def save_frontier(self, population: list[Individual], gen: int) -> None:
        ...
        for ind in front:
            payload.append({
                ...
                "summary": ind.summary,
                "penalty": ind.penalty,
                "outliers": ind.outliers,
                "feature_stats": ind.features_stats,
                "genes": self._serialize_genes(ind),
            })
