...
COMBINED_PATHS = {
    'S1': Path('CoOpt/results/S1_combined_frontier.json'),
    'S2': Path('CoOpt/results/S2_combined_frontier.json'),
}
        ...
        for label, info in labels.items():
            ...
            for _, row in df.iterrows():
                ...
                rows.append({
                    ...
                    'row': pos['row'],
                    'col': pos['col'],
                    'cooling_kwh_per_m2': row.get('cooling_kwh_per_m2_pred', None),
                    'heating_kwh_per_m2': row.get('heating_kwh_per_m2_pred', None),
                    'other_electricity_kwh_per_m2': row.get('other_electricity_kwh_per_m2_pred', None),
                    'multi_family_ha': row.get('multi_family_ha', None),
                    'facility_industrial_ha': row.get('facility_industrial_ha', None),
                    'road_area_ha': row.get('road_area_ha', None),
                    'parks_green_ha': row.get('parks_green_ha', None)
                })
            out = {
                'scenario': scenario,
                'label': label,
