...
def main() -> None:
    ...
    for combo in COMBOS:
        ...
        record = {
            ...
            "floor_high": combo["floor_high"],
            "count": count,
            "dev_min": dev_stats.get("min"),
            "dev_max": dev_stats.get("max"),
            "dev_mean": dev_stats.get("mean"),
            "energy_min": energy_stats.get("min"),
            "energy_max": energy_stats.get("max"),
            "energy_mean": energy_stats.get("mean"),
        }
        results.append(record)
        print(
            f"{record['name']}: count={record['count']}, "
            f"dev_mean={record['dev_mean']}, energy_mean={record['energy_mean']}"
        )
